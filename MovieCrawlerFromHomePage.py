...
def get_movie_list(url, decode_type='utf-8'):
    movie_link_list = Common.CrawlerToHTML.get_links_from_html_keyword(url, '/html/gndy/dyzz/2016', decode_type)
...
def main():
    ...
    search_movie_download_list = Common.CommonMovieCrawler.get_movie_download_list(my_real_movie_link_list, 'gbk',
                                                                                   False)
    Common.FileCommon.write_result_to_txt(search_movie_download_list, os.path.abspath('.') + '/top_movies',
                                          'movies2016.txt')
